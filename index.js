...
const sql = neon(
  'postgresql://piscolita_owner:qg0uBlwk4vLc@ep-withered-silence-a5uth5dy.us-east-2.aws.neon.tech/piscolita?sslmode=require'
);
...
app.get('/Equipo', async (req, res) => {
  ...
  const lista = await sql(
    "SELECT * FROM products WHERE categ = 'Equipamiento'"
  );
...
app.post('/LogAdmin', async (req, res) => {
  ...
  try {
    ...
    if (match) {
      ...
      const token = jwt.sign({ id, role: 'admin', exp: FMFN }, SPW);
...
app.post('/login', async (req, res) => {
  ...
  if (bcrypt.compareSync(password, hash)) {
    ...
    const token = jwt.sign({ id, exp: FMFN }, SPW);
...
app.post('/producti', async (req, res) => {
  ...
  const query =
    'INSERT INTO products (name, price, foto, categ, db, descr) VALUES ($1, $2, $3, $4, $5, $6)';
...
app.post('/carrito/add', async (req, res) => {
  ...
  if (!userId) {
    res
      .status(400)
      .send('Necesitas logearte para agregar cosas a un carrito :P');
    return;
  }
  ...
  try {
    ...
    if (results.length > 0) {
      const queryUpdate =
        'UPDATE cart SET quantiti = quantiti + $1 WHERE us_id = $2 AND prod_id = $3';
      await sql(queryUpdate, [quantiti, userId, prod_id]);
    } else {
      const queryInsert =
        'INSERT INTO cart (quantiti, us_id, prod_id) VALUES ($1, $2, $3)';
...
app.post('/pagar', async (req, res) => {
  ...
  const userId = req.cookies.userId;
  const querrrry = 'SELECT wallet FROM users WHERE id = $1';
  const wallet = await sql(querrrry, userId);
  try {
    const querry = 'UPDATE users SET wallet = wallet - $1 WHERE id = $2';
    const query = 'INSERT INTO ttl_v (ttl_venta) VALUES ($1)';
    const querrry = 'DELETE FROM cart WHERE id = $1';
    if (wallet >= total) {
      await sql(query, total);
      await sql(querry, [total, UsId]);
      await sql(querrry, UsId);
      res.redirect('/carrito');
    } else {
      res.status(500).send('No te alcanza la plata');
    }
  } finally {
...
app.post('/recargar-saldo', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: 'Error al recargar saldo' });
  }
});
